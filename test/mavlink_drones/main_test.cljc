(ns mavlink-drones.main-test
  "Contract + behavioral test for the mavlink-drones L5 actor (cljc port).
...
(deftest healthz
  (let [[body status] (m/healthz)]
    (is (= 200 status)) (is (= "mavlink_drones-compat" (:actor body))) (is (= (set m/entities) (set (:entities body))))))

#?(:clj (defn -main [& _]
          (let [{:keys [fail error]} (run-tests 'mavlink-drones.main-test)]
