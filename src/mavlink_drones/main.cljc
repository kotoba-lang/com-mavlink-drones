(ns mavlink-drones.main
  "Kotodama WASM entrypoint for the Mavlink_drones clean-room actor (L5) — Clojure port.
  ...
  facts (`mavlink_drones.<Entity>/<field>`); `*store*` is the in-memory materialization
  used by the contract test and by the WASM runtime before a live engine binds."
  (:require [clojure.string :as str]))

(def ns-prefix "mavlink_drones")
(def tier "L5")
...
(def entity-specs
  [{:entity "Heartbeat"         :plural "heartbeats"         :id-prefix "mavlinkd_hea"
    :fields [:type :autopilot :baseMode :customMode :systemStatus :mavlinkVersion]  :required [:type :autopilot]
    :coerce {:type :int :autopilot :int :baseMode :int :customMode :int :systemStatus :int :mavlinkVersion :int}  :refs {}}
   {:entity "SysStatus"         :plural "sysstatuses"        :id-prefix "mavlinkd_sys"
    :fields [:load :voltageBattery :currentBattery :batteryRemaining :dropRateComm :errorsComm]  :required [:load :voltageBattery]
    :coerce {:load :int :voltageBattery :int :currentBattery :int :batteryRemaining :int :dropRateComm :int :errorsComm :int}  :refs {}}
   {:entity "GpsRawInt"         :plural "gpsrawints"         :id-prefix "mavlinkd_gps"
    :fields [:timeUsec :fixType :lat :lon :alt :vel :satellitesVisible]  :required [:timeUsec :fixType]
    :coerce {:timeUsec :int :fixType :int :lat :int :lon :int :alt :int :vel :int :satellitesVisible :int}  :refs {}}
   {:entity "Attitude"          :plural "attitudes"          :id-prefix "mavlinkd_att"
    :fields [:timeBootMs :roll :pitch :yaw :rollspeed :pitchspeed :yawspeed]  :required [:timeBootMs :roll]
    :coerce {:timeBootMs :int :roll :float :pitch :float :yaw :float :rollspeed :float :pitchspeed :float :yawspeed :float}  :refs {}}
   {:entity "GlobalPositionInt" :plural "globalpositionints" :id-prefix "mavlinkd_glo"
    :fields [:timeBootMs :lat :lon :alt :relativeAlt :vx :vy :vz :hdg]  :required [:timeBootMs :lat]
    :coerce {:timeBootMs :int :lat :int :lon :int :alt :int :relativeAlt :int :vx :int :vy :int :vz :int :hdg :int}  :refs {}}
   {:entity "MissionItem"       :plural "missionitems"       :id-prefix "mavlinkd_mis"
    :fields [:targetSystem :targetComponent :seq :frame :command :current :autocontinue :x :y :z :missionType]  :required [:targetSystem :targetComponent]
    :coerce {:targetSystem :int :targetComponent :int :seq :int :frame :int :command :int :current :int :autocontinue :int :x :float :y :float :z :float :missionType :int}  :refs {}}])
...
(defn emit-facts
  "EAVT facts for one record: {\"mavlink_drones.<Entity>/<field>\" v ...}. The datomic
...
(defn healthz [] [{:status "ok" :actor "mavlink_drones-compat" :tier tier :entities entities} 200])
...
(defn start! [] :mavlink_drones-compat/ready)
